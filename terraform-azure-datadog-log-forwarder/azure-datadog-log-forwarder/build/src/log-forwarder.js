...
class Scrubber {
    constructor(context, configs) {
        ...
        configs.forEach(config => {
            try {
                this.rules.push(new ScrubberRule(config.name, config.rule.pattern, config.rule.replacement));
            }
            catch (_a) {
                context.log.error(`Regexp for rule ${config.name} pattern ${config.rule.pattern} is malformed, skipping. Please update the pattern for this rule to be applied.`);
            }
        });
    }
    scrub(record) {
...
class EventhubLogForwarder {
    ...
    async formatLogAndSend(messageType, record) {
            ...
            return this.send(record)
                ...
                setTimeout(() => {
                    this.send(record)
                        ...
                        .catch(err => {
                        this.context.log.error(`unable to send request after 2 tries, err: ${err}`);
                        reject();
                    });
                }, 1000);
            });
    ...
    async handleLogs(logs) {
        ...
        switch (logsType) {
            ...
            case JSON_OBJECT:
                ...
                break;
            case STRING_ARRAY:
                logs.forEach((log) => promises.push(this.formatLogAndSend(STRING_TYPE, log)));
                break;
            case JSON_ARRAY:
    ...
    async handleJSONArrayLogs(logs, logsType) {
        let promises = [];
        logs.forEach((message) => {
            ...
            if (message.records != undefined) {
                message.records.forEach((message) => promises.push(this.formatLogAndSend(JSON_TYPE, message)));
            }
            else {
    ...
    extractMetadataFromResource(record) {
        ...
        if (resourceId[0] === 'subscriptions') {
            ...
            if (resourceId.length > 3 && resourceId[3]) {
                ...
                metadata.source = this.formatSourceType(resourceId[3]).replace('aadiam', 'activedirectory');
            }
        }
        return metadata;
    }
...
//# sourceMappingURL=log-forwarder.js.map